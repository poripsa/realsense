							 ...
							 int temporal_table_idx, float temporal_alpha, float temporal_delta)
{
	...
	check_error(e);
  ROS_INFO("SetPostProcessingParams");

	rs2_set_option((rs2_options *)g_spatial_filter_block, RS2_OPTION_FILTER_SMOOTH_DELTA, spatial_delta, &e);
	check_error(e);

	// temporal
	rs2_set_option((rs2_options *)g_temporal_filter_block, RS2_OPTION_FILTER_MAGNITUDE, (float)temporal_table_idx, &e);
	check_error(e);
