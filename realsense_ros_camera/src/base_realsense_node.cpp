...
void BaseRealSenseNode::publishITRTopic(sensor_msgs::PointCloud2& msg_pointcloud, bool colorized_pointcloud)
{
  ...
  float min_depth_threshold = 0.f;
  float max_depth_threshold = 5.f;

  if (colorized_pointcloud)
  {
    sensor_msgs::PointCloud2Iterator<float>iter_x(msg_pointcloud, "x");
    sensor_msgs::PointCloud2Iterator<uint8_t>iter_rgb(msg_pointcloud, "rgb");
    ...
    for (int y = 0; y < depth_intrinsics.height; ++y)
    {
      for (int x = 0; x < depth_intrinsics.width; ++x)
      {
        ...
        iter_x[0] = depth_point[0];
        iter_x[1] = depth_point[1];
        iter_x[2] = depth_point[2];
            ...
            || color_pixel[0] < min_depth_threshold || color_pixel[0] > color_intrinsics.width)
        {
          ...
          iter_rgb[2] = static_cast<uint8_t>(96);
          iter_rgb[1] = static_cast<uint8_t>(157);
          iter_rgb[0] = static_cast<uint8_t>(198);
        }
        else
        {
          ...
          iter_rgb[2] = static_cast<uint8_t>(color_data[offset]);
          iter_rgb[1] = static_cast<uint8_t>(color_data[offset + 1]);
          iter_rgb[0] = static_cast<uint8_t>(color_data[offset + 2]);
        }
